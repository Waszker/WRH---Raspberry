...
class KankunSocketModule(base_module.Module):
    """
    This class works with Kankun WiFi socket.
    """
    type_number = 4
    type_name = "KANKUN WIFI SOCKET"

    def __init__(self, configuration_file_line=None):
        base_module.Module.__init__(self, configuration_file_line)
        self.type_number = KankunSocketModule.type_number
        self.type_name = KankunSocketModule.type_name
    ...
    def get_module_description(self):
        ...
        """
        return "Kankun WiFi socket - electrical socket with WiFi connection capabilities"
    ...
    def run_registration_procedure(self, device_id, device_token):
        ...
        self.gpio = u.get_non_empty_input("Please input Kankun WiFi socket IP address: ")
    ...
    def edit(self, device_id, device_token):
        ...
        new_gpio = raw_input("Please input new IP address of kankun device: ")
    ...
    def start_work(self, device_id, device_token):
        ...
        send "ON" or "OFF" message which in turn sets socket state.
        """
        host = ''
        port = self.address
        ...
        while True:
            ...
            elif str(data) == "OFF" or str(data) == "off":
    ...
    def _set_socket_state(self, should_turn_on):
        ...
        url = "http://" + self.gpio + "/cgi-bin/relay.cgi?" + state
        request = urllib2.Request(url)
        urllib2.urlopen(request).read()


if __name__ == "__main__":
    print 'Kankun WiFi: started.'
    device_line = sys.argv[1]
    conf_line = sys.argv[2]

    device_info = c.get_device_entry_data(device_line)
    kankun = KankunSocketModule(conf_line)
    kankun.start_work(device_info[0], device_info[1])
